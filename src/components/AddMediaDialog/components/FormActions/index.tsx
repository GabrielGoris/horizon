...
export function FormActions({ isCatalogSelectionLoading, isSubmitting, onCancel, onSubmitWithPriority }: FormActionsProps) {
  return (
    <div className="shrink-0 border-t border-white/5 bg-[#1a1a1e] px-8 py-6">
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end sm:gap-4">
        ...
        >
          {isCatalogSelectionLoading ? "Carregando dados..." : isSubmitting ? "Guardando..." : "Guardar Obra"}
        </button>
        <button
          type="button"
          disabled={isSubmitting}
        ...
        >
          {isCatalogSelectionLoading ? "Carregando dados..." : "Adicionar a lista de prioridade"}
        </button>
      </div>
    </div>
  );
